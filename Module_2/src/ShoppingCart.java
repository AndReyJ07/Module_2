...
public class ShoppingCart {
    ...
    public static void main(String[] args){
        ...
        assertEquals(0, calculateDiscount(ItemType.NEW, 1));
        assertEquals(1, calculateDiscount(ItemType.REGULAR, 10));
        assertEquals(80, calculateDiscount(ItemType.REGULAR, 1000));
        assertEquals(0, calculateDiscount(ItemType.SECOND_FREE, 1));
        assertEquals(51, calculateDiscount(ItemType.SECOND_FREE, 10));
        assertEquals(80, calculateDiscount(ItemType.SECOND_FREE, 1000));
        assertEquals(70, calculateDiscount(ItemType.SALE, 1));
        assertEquals(71, calculateDiscount(ItemType.SALE, 10));
        assertEquals(80, calculateDiscount(ItemType.SALE, 1000));
    }
    /**
     * Adds new item.
     *
    ...
    public String formatTicket(){
        ...
        // header
        makeHeader(header, align, width, sb);
        // separator
        makeSeparator(lineLength, sb);
        // lines
        for (String[] line : lines) {
            makeFooter(align, line, width, sb);
            sb.append("\n");
        }
        if (lines.size() > 0) {
            // separator
        ...
        // footer
        makeFooter(align, footer, width, sb);
        return sb.toString();
    }

    private void makeFooter(int[] align, String[] footer, int[] width, StringBuilder sb) {
        for (int i = 0; i < footer.length; i++)
    ...
    private void makeHeader(String[] header, int[] align, int[] width, StringBuilder sb) {
        for (int i = 0; i < header.length; i++)
